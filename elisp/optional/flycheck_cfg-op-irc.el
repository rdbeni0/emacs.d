...
(use-package erc
  ...
  (defun cfg/erc//servers (server-list)
    "Connect to every server listed in SERVER-LIST."
  ...
  (defun cfg/erc-default-servers ()
    "Connect to all servers defined in `erc-server-list'."
    (interactive)
    (if erc-server-list
  ...
  (define-advice erc-track-find-face (:around (orig-fun &rest args) promote-query)
    "Highlight query buffers as if they contained the current nick."
