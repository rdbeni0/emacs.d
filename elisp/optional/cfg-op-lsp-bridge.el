;;; cfg-op-lsp-bridge.el --- clean lsp-bridge configuration -*- lexical-binding: t -*-
;;; Commentary:
;;
...
(use-package lsp-bridge
  ...
  ;; opcjonalnie: inlay hints tylko w python
  ...
  ;; --------------------------------------
  ;; Disable LSP in selected modes:
  ;; --------------------------------------

  ;; turn OFF multiserver for python and use only default langserver:
  ...
  (dolist (hook '(web-mode-hook
                  ...
                  perl-mode-hook
                  cperl-mode-hook))
    (add-hook hook (lambda () (lsp-bridge-mode -1)) t))


  ;; --------------------------------------
  ;; ENVRC (direnv) — tutaj restart ma sens
  ;; --------------------------------------
