...
unset HISTFILE
sed -i '/exec env TERM=.dumb. INSIDE_EMACS/d' ~/.bash_history
...
(defun tramp-term--select-host ()
  ...
  (let ((crm-separator "@"))
    (completing-read-multiple "[user@]host: " (delete nil (tramp-term--parse-hosts "~/.ssh/ssh_config")))))

(defun tramp-term--parse-hosts (ssh-config)
  "Parse any host directives from SSH-CONFIG file and return them
as a list of strings"
  (mapcar 'cadr (remove nil (delete nil (tramp-parse-sconfig ssh-config)))))
