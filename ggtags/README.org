...
And finally merge some parsers into one label - add this somewhere at the beginning:
...
*universaltags* seems to be most updated and written in =C= (it means fastest one) - so that one we will use with higher priority.
